...
def optimize(optimizer, log_prob, q_values, next_q_values, action, next_action, reward, done, gamma):
    current_q_action = q_values[0, action]
    next_q_next_action = next_q_values[0, next_action].detach()
    
        
    target = reward + (gamma * next_q_next_action.detach() * (1 - int(done)))    
    actor_loss = -log_prob * current_q_action.detach()
    
    critic_loss = F.mse_loss(current_q_action, target)
